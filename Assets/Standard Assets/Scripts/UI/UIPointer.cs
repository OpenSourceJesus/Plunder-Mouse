...
public class UIPointer : MonoBehaviour
{
	...
	private static UIPointer _instance;
	...
	{ 
		...
		get
		{
			if (_instance == null)
			{
				...
				_instance = (UIPointer)GameObject.Instantiate((UIPointer)Resources.Load("Prefabs/GazePointerRing", typeof(UIPointer)));
			}
			return _instance;
		}
	...
	void Update ()
	{
		...
		transform.position = OVRCameraRig.CurrentHand.position + OVRCameraRig.CurrentHand.forward * depth;
	...
	public void SetPosition(Vector3 pos, Vector3 normal)
	{
		...
		depth = (OVRCameraRig.CurrentHand.position - pos).magnitude;
		
        //set scale based on depth
		currentScale = depth * depthScaleMultiplier;
		this.GetComponent<TrailRenderer>().widthMultiplier = this.currentScale;
	...
	{
		SetPosition(pos, OVRCameraRig.CurrentHand.forward);
	}
	...
	void LateUpdate()
	{
		...
		if (positionSetsThisFrame == 0)
		{
			...
			Quaternion newRot = transform.rotation;
			newRot.SetLookRotation(OVRCameraRig.CurrentHand.forward, (lastPosition - transform.position).normalized);
			transform.rotation = newRot;
		}
