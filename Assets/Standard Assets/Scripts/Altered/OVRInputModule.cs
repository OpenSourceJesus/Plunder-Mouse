...
namespace UnityEngine.EventSystems
{
    public class OVRInputModule : PointerInputModule
    {
	    ...
	    // public PlunderMouse.InputManager.VRButtonGroup clickVrButtonGroup;
       ...
       // private OVRRaycaster _activeGraphicRaycaster;
        ...
        private bool SendSubmitEventToSelectedObject()
        {
            ...
            // if (InputManager.inputter.GetButtonDown(m_SubmitButton))
            //     ExecuteEvents.Execute(eventSystem.currentSelectedGameObject, data, ExecuteEvents.submitHandler);

            // if (InputManager.inputter.GetButtonDown(m_CancelButton))
        ...
        {
            // bool allow = InputManager.inputter.GetButtonDown(m_HorizontalAxis);
            // allow |= InputManager.inputter.GetButtonDown(m_VerticalAxis);
        ...
        private Vector2 GetRawMoveVector()
        {
            ...
            // move.x = InputManager.inputter.GetAxisRaw(m_HorizontalAxis);
            // move.y = InputManager.inputter.GetAxisRaw(m_VerticalAxis);

            // if (InputManager.inputter.GetButtonDown(m_HorizontalAxis))
            // {
            //     if (move.x < 0)
            //         move.x = -1f;
            //     if (move.x > 0)
            //         move.x = 1f;
            // }
            // if (InputManager.inputter.GetButtonDown(m_VerticalAxis))
            // {
            //     if (move.y < 0)
            //         move.y = -1f;
            //     if (move.y > 0)
...
#region PointerEventData pool
        ...
        protected MouseState GetGazePointerData()
        {
	        ...
	        leftData.worldSpaceRay = new Ray(OVRCameraRig.CurrentHand.position, OVRCameraRig.CurrentHand.forward);
            ...
            if (ovrRaycaster) 
            {
                ...
                if (graphicRect != null)
                {
	                ...
	                UIPointer.instance.SetPosition(worldPos, normal);
                    // Make sure it's being shown
                    UIPointer.instance.RequestShow();
                }
            }
            ...
            if (physicsRaycaster)
            {
                ...
                UIPointer.instance.RequestShow();
                UIPointer.instance.SetPosition(raycast.worldPosition, raycast.worldNormal);
            }
        ...
        {
	        // bool pressed = InputManager.inputter.GetButtonDown(clickButton) || clickVrButtonGroup.GetDown();
	        // bool released = InputManager.inputter.GetButtonUp(clickButton) || clickVrButtonGroup.GetUp();

// #if UNITY_ANDROID && !UNITY_EDITOR
//             pressed |= InputManager.inputter.GetMouseButtonDown(0);
//             released |= InputManager.inputter.GetMouseButtonUp(0);
// #endif
